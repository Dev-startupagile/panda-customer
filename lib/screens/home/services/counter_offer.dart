...
class _CounterOfferState extends State<CounterOffer> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) => {
      context.read<EstimateProvider>().getEstimateById(context, widget.requestId)
    });
  ...
  Widget build(BuildContext context) {
    ...
    void decline() async{
      ...
      await context.read<EstimateProvider>().declineEstimate(context,context.read<EstimateProvider>().estimateDetail[0].id);
      context.read<NotificationProvider>().updateCounterOffer();
    }

    void accept() async{
      await context.read<EstimateProvider>().approveEstimate(context, context.read<EstimateProvider>().estimateDetail[0].id);
      context.read<NotificationProvider>().updateCounterOffer();
    }
          ...
          child: Card(
              child: Column(
                children: [
                  ...
                  ListTile(
                      title: const Text("Offer Description", style: KAppTitleTextStyle,),
                      subtitle: Row(
                        children: [
                          const Icon(Icons.circle,color: kPrimaryColorSecondary,),
                          Text("Technician Sent You New Offer",style: KLatoRegularTextStyle,)
                        ],
                      ),
                  ),
                  const SizedBox(
                            ...
                            itemBuilder: (context,index){
                              ...
                              return  Column(
                                children: [
                                  ...
                                  Row(
                                    ...
                                    children: [
                                      Expanded(
                                        child: Center(
                                            child:
                                            Text(req.note ?? "",style: KHintTextStyle)
                                        ),
                                      ),
                                              ...
                                              child: Container(
                                                  ...
                                                  child: Row(
                                                      ...
                                                      children: [
                                                        ...
                                                        Row(children: [
                                                          Text(req.items[index].title ?? "",style: const TextStyle(fontSize: 16,fontWeight: FontWeight.normal)),

                                                        ]),
                                                        Row(children: [
                                                          Text("\$${req.items[index]. price}",style: const TextStyle(fontSize: 16,fontWeight: FontWeight.normal)),
                                  ...
                                  customRequestList("Labour Estimation", '\$${req.totalEstimation }'),

                                  Visibility(
                                      visible: req.discount != 0,
                                      child: customRequestList("Discount", '\$${req.discount ?? 0}')
                                  ),
