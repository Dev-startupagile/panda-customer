...
class RequestOffer extends StatefulWidget {
  String requestId;
  RequestOffer({Key? key, required this.requestId  }) : super(key: key);
...
class _RequestOfferState extends State<RequestOffer> {
  ...
  Widget build(BuildContext context) {
    ...
    void decline() async{
      ...
      await context.read<ServiceRequestProvider>().cancelServiceRequest(context,widget.requestId);
      context.read<NotificationProvider>().updateRequestOffer();
    }

    void accept() async{
      await context.read<ServiceRequestProvider>().acceptServiceRequest(context, widget.requestId);
      context.read<NotificationProvider>().updateRequestOffer();
    }
            ...
            child: Card(
                child: Column(
                    children: [
                      ...
                      ListTile(
                        title: const Text("Request Detail", style: KAppTitleTextStyle,),
                        subtitle: Row(
                          children: [
                            const Icon(Icons.circle,color: kPrimaryColorSecondary,),
                            Text("Technician Re Schedule Request",style: KLatoRegularTextStyle,)
                          ],
                        ),
                      ),
                      const SizedBox(
                      ...
                      Consumer<ServiceRequestProvider>(
                          builder: (context,value,child) {
                            ...
                            Column(
                              children: [
                                customRequestList("Service Type",value.requestDetail?.description.title),

                                customRequestList("Service Description", value.requestDetail?.description.note ?? ""),
